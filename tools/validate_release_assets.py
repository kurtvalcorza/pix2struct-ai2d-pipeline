"""Static release-asset validation for the Pix2Struct AI2D-base DIMER pipeline.
...
EXPECTED_MODEL_ID = "google/pix2struct-ai2d-base"
...
KNOWN_SHAS: frozenset[str] = frozenset(
    ("c83a9b9692933aff8349157c88a413df9d02c4e5",)  # the pinned AI2D dataset revision, cited beside the model revision
)
BYOD_GATES = ("USE_BYOD",)
EXPECTED_OUTPUTS = (
    "outputs/pix2struct_ai2d_train.jsonl",
    "outputs/pix2struct_ai2d_input_manifest.json",
    "outputs/pix2struct_ai2d_evaluation_report.json",
    "outputs/pix2struct_ai2d_answers.csv",
    "outputs/pix2struct_ai2d_adapter",
    "outputs/pix2struct_ai2d_result.json",
)
CODE_MARKERS = (
    # Stage 4: the pinned shard, diagram-disjoint split, validation, JSONL, refusal probes
    "USE_BYOD = False",
    ...
    # Stage 5: ceilings, the inference contract with its manifest, probe, sanity checks and the drawn diagram
    ...
    # Stage 6: chance, two baselines and the frozen model on the test split, per category
    ...
    # Stage 9: the diagram re-answered, artifact, reload parity, provenance
    ...
    "'weight_format': 'SafeTensors (bfloat16, upcast to float32), digest-verified'",
...
MARKDOWN_MARKERS = (
    "**Capability:** diagram multiple-choice question answering and bounded supervised fine-tuning",
    "**Apache-2.0**",
    "**No score exists**",
    "**not evidence that it is the right one**",
    "**adaptation with labelled questions**",
    "**rendered as a text header above the diagram**",
    "**one pinned parquet shard**",
    "**chance**",
    "**position-prior baseline**",
    "**longest-option baseline**",
    "**unmatched rate**",
    "**no dispersion estimate**",
    "Free-form or open-ended answers (the contract matches the generated text",
    "**Weight-format note:**",
)
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    564_670_657: "upstream pytorch_model.bin at google/pix2struct-ai2d-base@0d6b260 (not staged)",
}

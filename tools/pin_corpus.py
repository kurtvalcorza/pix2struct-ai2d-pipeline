"""Record the pins of the AI2D sample shard in src/pix2struct_ai2d_pipeline/samples.py.

Needs Hub access. Downloads `CORPUS_FILE["path"]` of `CORPUS_REPO` at the immutable `CORPUS_REVISION`,
checks its size against the committed pin, computes its SHA-256, counts its rows and distinct diagrams,
rewrites the `sha256`, `rows` and `images` entries of `CORPUS_FILE`, and prints the realised split counts
...
carried module changed. Run it once; re-running on an already-pinned file only confirms the pins.
    ...
    python tools/pin_corpus.py [--cache weights/ai2d]
"""
...
from pix2struct_ai2d_pipeline import samples  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    parser.add_argument("--cache", default=str(ROOT / "weights" / "ai2d"))
    ...
    images = len({hashlib.sha256(r["image_bytes"]).hexdigest()[:16] for r in rows})
    ...
    module = ROOT / "src" / "pix2struct_ai2d_pipeline" / "samples.py"
    ...
    samples.CORPUS_FILE.update({"sha256": digest, "rows": len(rows), "images": images})
    splits = samples.build_sample_dataset(rows, image_dir=cache / "images")
    print({"sha256": digest, "rows": len(rows), "images": images})
    for name, part in splits.items():
        print({name: {"questions": len(part), "diagrams": len({r["image_id"] for r in part})}})
